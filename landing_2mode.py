...
class Lander(lcvx.Problem,object):
    def __init__(self,micp=False):
        ...
        add_constraint([x[k+1] == A*x[k]+B*sum([u[i][k] for i in range(self.M)])+w for k in range(self.N)],'nu_x')
        ...
        for i in range(self.M):
            ...
            add_constraint([gamma[i][k]*self.rho1[i] <= sigma[i][k] for k in range(self.N)],'lambda_rho1')
            add_constraint([sigma[i][k] <= gamma[i][k]*self.rho2[i] for k in range(self.N)],'lambda_rho2')
            if not micp:
...
ax.axis('equal')
ax.plot(xtraj[0],xtraj[1],**pos_style)
#for k in range(xtraj.shape[1]):
#    ax.plot([xtraj[0,k],xtraj[0,k]+v_whisker_scale*xtraj[2,k]],
#            [xtraj[1,k],xtraj[1,k]+v_whisker_scale*xtraj[3,k]],**vel_style)
for k in range(xtraj.shape[1]-1):
    for i in range(M):
        ax.plot([xtraj[0,k],xtraj[0,k]-u_whisker_scale*utraj[i][0,k]],
                [xtraj[1,k],xtraj[1,k]-u_whisker_scale*utraj[i][1,k]],**thrust_style[i])
...
def draw_polar_grid(ax,angle,bounds,direction,angle_ticks,magnitude_ticks,
    ...
    for mag in magnitude_ticks:
        if mag>bounds[0]+eps and mag<bounds[1]-eps:
            ...
            ax.plot(grid_line[0],grid_line[1],**grid_style)
            ax.text(grid_line[0][-1],grid_line[1][-1],'$%d$'%(mag),**text_style)
...
plt.tight_layout()
plt.axis('off')
ax.plot(0,0,color='black',linestyle='none',marker='.',markersize=5,zorder=100)
for i in range(M):
    draw_polar_grid(ax,theta[i],[rho1[i],rho2[i]],thrust_direction,angles,
                    magnitudes,border_style[i],grid_style[i])
    ax.plot([-u if la.norm(u)>np.sqrt(1e-5) else np.nan for u in utraj[i][0]],
            [-u if la.norm(u)>np.sqrt(1e-5) else np.nan for u in utraj[i][1]],**thrust_style[i])
