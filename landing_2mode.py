...
def draw_polar_grid(ax,angle,bounds,direction,angle_ticks,magnitude_ticks,
    ...
    ang_text_style = dict(va='bottom',ha='center',fontsize=10)
    mag_text_style = dict(va='center',ha='right',fontsize=10)
    mag_offset = 0.5
    ang_offset = 0.4
...
ax.autoscale(tight=True)
ax.set_xlabel('Normalized time $\hat t$')
ax.set_ylabel('$\sum_{i=1}^M\|u_i(t)\|_2$ [m/s$^2$]')
...
for i in range(M):
    ...
    ax.autoscale(tight=True)
    ax.set_xlabel('Normalized time $\hat t$')
    ax.set_ylabel('$\|u_{%d}(t)\|_2$ [m/s$^2$]'%(i+1))
...
y = misc['y']
beta2 = [dual['beta_2_%d'%(i+1)].flatten() for i in range(M)]
beta3 = [dual['beta_3_%d'%(i+1)].flatten() for i in range(M)]
...
if save_pdf:
    fig.savefig('figures/rocket_input_valuation.pdf',bbox_inches='tight',format='pdf',transparent=True)
    
#### Dual variable plots (support functions)
...
# ax = fig.add_subplot(111)
# for i in range(M):
#     ax.plot(t[:-1],cost_coeff[i],color=colors[i],linestyle='none',**dual_style)
#     ax.step(t[:-1],cost_coeff[i],color=colors[i],
#             label='$\hat\\beta_2^{%d}(t)-\hat\\beta_3^{%d}(t)$'%(i+1,i+1),where='post')
...
# 
# if save_pdf:
#     fig.savefig('figures/rocket_argmax_beta_diff.pdf',bbox_inches='tight',format='pdf',transparent=True)
# =============================================================================
...
if save_pdf:
    fig.savefig('figures/rocket_primer_vector.pdf',bbox_inches='tight',format='pdf',transparent=True)